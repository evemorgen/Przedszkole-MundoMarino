...
liczbaOstatnichTimeoutowBaza = 0
...
TIMEONSCREEN = 30   # ile czasu dane są na ekranie
...
NAZWA_RAMKA = PWD + "/ramka.png"	       #na raspi
#NAZWA_RAMKA = "ramka.png"			 #na maku
...
def getNameFromID(id):
    global liczbaOstatnichTimeoutowBaza
    czyJestId = 1
    while czyJestId:
        try:
            doZwrocenia = []
            url = 'http://192.168.1.204/index.php/api/card2name/id/' + id
            response = urllib2.urlopen(url,timeout=URLTIMEOUT)
            html = response.read()
            pocz = 0
            end = html.find('<br>')
            if end + 4 == len(html):
                str = unicode()
                doZwrocenia.append(unicode(html[0:end],"utf-8"))
                return doZwrocenia
            else:
                while end != -1:
                    doZwrocenia.append(unicode(html[pocz:end],"utf-8"))
                    pocz = end + 4
                    end = html.find('<br>',pocz)
                return doZwrocenia
            liczbaOstatnichTimeoutowBaza = 0
            czyJestId = 0
...
def printTimeLeft(timeLeft,poz=0):
    ...
    ramka = pygame.image.load(NAZWA_RAMKA)
...
while not done:                                                     # główna pętla, jak w AVR'ach, huh
    ...
    if flagaZmianyKarty == 1:                                       # jak była to zajebiście tylko trzea dodać dzieciaka
        ...
        first = lastCardReaded.find("-") + 2
        dane = getNameFromID(lastCardReaded[first:len(lastCardReaded)])                        # translacja nr na nazwisko
        if dane:
            for x in dane:
                nowaOsoba = Person(x)                                    # nowy obiekt klasy osoba, jak u szmuca, LOL
                if findOnList(nowaOsoba.name) == 0:                         # jeśli nie ma jeszcze na liście to WANIA DAWAJ
                    listaDoWypiania.append(nowaOsoba)                       # znaczy DODAWAJ.
    ...
    if liczbaOstatnichTimeoutowBaza > 0:
        screen.blit(pygame.font.Font(NAZWA_CZCIONKA,80).render(u"Brak połączenia z bazą danych!",True,(255,255,255)),(200,HEIGHT / 3 + 100))
